...
c      Hmin=-35000.0D0          !use specified minimum water suction for plant uptake and water movement.
     ...
     +    ipsw_depth,ipet,CKinit,CKmax,emitr0)
...
C
     ...
     +    TNDX(MXAPP),UDEPTH(MXAPP),IMP(MXAPP),ITL(MXAPP),
     +    ITOFF(MXAPP,2),IWTILL(MXAPP),NUMTL,ITPL(MXAPP)
C
      COMMON /KNTCS/ CONCX2(MXNOD,MXPEST),EK2(MXNOD,MXPEST),
     +    RK2(MXPEST),RDIF(MXPEST),FEK2(MXPEST),IEK2(MXPEST)
C
...
C
     ...
     +    JULBDAY,JULEDAY,niw,ipsw_depth,ipet,CKinit,CKmax,emitr0)
C
C======================================================================
C
...
C       ..SETUP LAST IMPLIMENT FOR ANHYDROUS APPLICATOR FOR USE BY BMP
     ...
     +        TNDX(J),ITL(J)
          ...
          Tillmixeffi1(j) = 100.0 - TNDX(J)*100.00
        ELSE
...
CZ-MA   Liwang Ma
      OMSEA(78)=TOTCO2+RCO2   ! DAILY NET CO2 RELEASE
