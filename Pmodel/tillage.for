...
C===============================================================================  
     ...
     +        .OR. Mansop(j)>0.0) THEN   
      ...
      Labp(1) = Labp(1) + Manwip(j)*Tillinceffi(Slnt)
      Labp(1) = Labp(1) + Manwop(j)*Tillinceffi(Slnt)
      Actp(1) = Actp(1) + Mansip(j)*Tillinceffi(Slnt)
      Actp(1) = Actp(1) + Mansop(j)*Tillinceffi(Slnt)
     ...
     +rated into the Labile and Active P pool of the First Soil layer'
      ...
      Mansop(j) = MAX(Mansop(j) - Mansop(j)*Tillinceffi(Slnt),0.0)
